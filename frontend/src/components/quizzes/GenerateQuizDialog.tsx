...
import { useGenerateQuiz } from '@/lib/hooks/use-quizzes'
import { chatApi } from '@/lib/api/chat'
import { sourcesApi } from '@/lib/api/sources'
import { notesApi } from '@/lib/api/notes'
import { BuildContextRequest, NoteResponse, SourceListResponse } from '@/lib/types/api'
import { QuizGenerationRequest } from '@/lib/types/quizzes'
...
import { QuizProfile } from '@/lib/types/quizzes'
...
/**
 * GenerateQuizDialog Component
 * 
 * Provides an interface for generating AI-powered quizzes based on notebook content.
 * Users can select specific sources and notes to include in the quiz generation.
 */
export function GenerateQuizDialog({ open, onOpenChange }: GenerateQuizDialogProps) {
  ...
  const quizTypes = [
    ...
    const [quizProfiles, setQuizProfiles] = useState<QuizProfile[]>([])

    useEffect(() => {
    if (open) {
        const fetchQuizProfiles = async () => {
        try {
            const profiles = await quizzesApi.getQuizProfiles()
            setQuizProfiles(profiles)
        } catch (error) {
            console.error('Failed to fetch quiz profiles', error)
            setQuizProfiles([
            { id: 'default', name: 'Default Profile', description: 'Standard quiz generation profile' }
            ])
        }
        }
        fetchQuizProfiles()
    ...
    const QUIZ_PROFILES = quizProfiles.map(profile => ({
        value: profile.id,
        label: profile.name
    }))
    ...
    const [quizProfile, setQuizProfile] = useState<string>('default')
  ...
  // Initialize selection defaults when content loads
  useEffect(() => {
    if (!open) {
      return
    }
  ...
  const handleSubmit = useCallback(async () => {
    if (!quizType) {
      toast({
        ...
        description: 'Select a quiz type before generating a quiz.',
        variant: 'destructive',
      })
      return
    }
    ...
    if (!quizTitle.trim()) {
      toast({
        ...
        description: 'Provide a title for the quiz.',
        variant: 'destructive',
      })
      return
    }
    ...
    try {
      ...
      if (!content.trim()) {
        toast({
          title: 'Add context',
          description: 'Select at least one source or note to include in the quiz.',
          variant: 'destructive',
        })
        return
      }
      ...
      const payload: QuizGenerationRequest = {
            ...
            additional_instructions: instructions.trim() ? instructions.trim() : undefined,
        }

      await generateQuiz.mutateAsync(payload)
  ...
  const isSubmitting = generateQuiz.isPending || isBuildingContext
    ...
    <Dialog open={open} onOpenChange={(value) => {
      ...
      <DialogContent className="w-[80vw] max-w-[1080px] max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the content to include and configure the quiz details before generating a new assessment.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Pick notebooks, sources, and notes to include in this quiz.
                </p>
              </div>
            ...
            <div className="rounded-lg border bg-muted/30">
              {notebooksQuery.isLoading ? (
                ...
                <div className="p-6 text-sm text-muted-foreground">
                  No notebooks found. Create a notebook and add content before generating a quiz.
                </div>
              ) : (
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                <Label htmlFor="quiz_profile">Quiz Profile</Label>
                <Select
                    value={quizProfile}
                    onValueChange={setQuizProfile}
                >
                    <SelectTrigger id="quiz_profile">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Recommended: 10-20 questions for optimal learning experience
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Specific topics to focus on, difficulty level, or any special requirements..."
                    rows={4}
                  />
                  <p className="text-xs text-muted-foreground">
                    Provide additional guidance to customize the quiz content and difficulty.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <p className="text-xs text-muted-foreground">
                The quiz will appear in your Quizzes list once generation is complete. 
